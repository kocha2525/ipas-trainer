/* ============================================================
   完全版 script.js（補足説明対応 / 正解・不正解どっちでも詳細カード）
   ============================================================ */
...
function showQuestion() {
    ...
    if (currentIndex >= currentWords.length) {
        ...
        // ★グループ選択に戻る（新規追加）
        document.getElementById("backToGroupBtn").onclick = () => {
            showScreen("groupScreen");
        };
...
function generateChoices(item) {
    ...
    while (list.length < 4) {
        const r = currentWords[Math.floor(Math.random() * currentWords.length)].term;
...
function showDetailCard(item) {
    ...
    detailArea.innerHTML += `
        <div class="card" style="margin-top:15px; background:#F0FAFF;">
            <div><strong>用語：</strong> ${item.term}</div>
            <div><strong>意味：</strong> ${item.meaning}</div>
        </div>
    `;
}
...
document.getElementById("historyBackBtn").onclick = () => {
    showScreen("mainScreen");
    showQuestion();  // ★現在の currentIndex の問題を再表示
};
